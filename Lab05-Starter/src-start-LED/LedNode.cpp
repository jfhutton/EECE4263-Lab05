...
void sendLedStatusMessage(String senderID, String ledStatus, String ledStatusMessage);
...
void setup() {
  ...
  if (!psClient.connected()) {
    Serial.println("In Setup and appear to have lost connection...reconnecting");
    reconnect();
  }
...
void loop()
{
  ...
  if (!psClient.connected()) {
    ...
    Serial.println("In Loop and appear to have lost connection...reconnecting"); // Debugging
    reconnect();
  }
...
void processMQTTMessage(char* topic, byte* json_payload, unsigned int length)
{
  ...
  if(strcmp(topic, sbuf) == 0) {
    ...
    if(!error) {
      ...
      Serial.println(); Serial.print("cmd = "); Serial.println(cmd);
      ...
      if(cmd == cmdOn) {
        ...
        // send an MQTT ledStatus message back to sending node
        sendLedStatusMessage(senderID, "off", "And darkness fell upon the land...");
      }
      else {
